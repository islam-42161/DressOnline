...
const DotsCarousel = ({ onGotoIndex, dots, animatedScrollX, style }) => {
  ...
  const Dot = ({ index }) => {
    ...
    const dotStyle = useAnimatedStyle(() => {
      ...
      const color = interpolateColor(animatedScrollX.value, [(index - 1) * width, index * width, (index + 1) * width], ['rgba(0,0,0,0.5)', 'rgba(0,0,0,1)', 'rgba(0,0,0,0.5)'])
      return {
    ...
    const outerpart = useAnimatedStyle(() => {
      ...
      const elevate = withTiming(interpolate(animatedScrollX.value, [(index - 1) * width, index * width, (index + 1) * width], [0, 5, 0], Extrapolate.CLAMP))
      return {
        transform: [
          {
            scale: scale
