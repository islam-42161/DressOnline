...
const ReadmoreCustom = ({descriptiveText,textstyle,numberOfLines}) => {
  ...
  return (
    ...
    >
    <Text onTextLayout={onTextLayout} style={textstyle} numberOfLines={show?numberOfLines:undefined}>{descriptiveText}</Text>
    </ScrollView>
    {lengthMore && (
      <Text style={[textstyle,{fontSize:12,color: '#3ca98b', backgroundColor: '#e2f9de',paddingHorizontal:5,alignSelf:'flex-start',marginTop:10,borderRadius:5}]} onPress={()=>setShow(!show)}>{show?"Read more":"Read less"}</Text>
    )}
    </View>
  )
}
